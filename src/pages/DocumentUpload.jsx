...
const DocumentUpload = () => {
    ...
    fileList.forEach(file => {
      ...
      if (!SUPPORTED_FORMATS.includes(extension)) {
        toast({
          ...
          description: `The file "${file.name}" was ignored. Only ${SUPPORTED_FORMATS.join(', ')} files are supported.`,
          variant: "destructive"
        });
        return;
      }
      ...
      acceptedFiles.push({ id: Date.now() + Math.random(), file, name: file.name, size: file.size });
    });
  ...
  const handleUpload = async () => {
    ...
    toast({ title: "Upload successful!", description: `${files.length} document(s) are now being analyzed.` });
    navigate('/dashboard');
  };
  ...
  return (
    <>
      <Helmet>
        <title>Upload Document - PlagiarismGuard</title>
        <meta name="description" content="Upload your documents for AI-powered plagiarism detection and analysis." />
      </Helmet>
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Drag and drop files or click to browse. Max 10MB per file.</CardDescription>
            </CardHeader>
            <CardContent>
              <div
            ...
            >
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Review your files before starting the analysis.</CardDescription>
                </CardHeader>
                <CardContent>
